...
[![版本](https://img.shields.io/badge/version-0.3.5-blue.svg)](https://github.com/user/pi-switch/releases)
...
`[proxy]` 徽章表示该配置被设置为通过本地透明代理路由。当一个配置被设为代理目标时，其 baseUrl 会自动指向代理服务器（如 `http://127.0.0.1:8190`），所有请求都会透明地通过代理路由，并自动进行故障转移和断路器保护。

使用代理的步骤：
1. 设置目标配置：`pi-switch proxy target <名称>`
2. 配置故障转移链：`pi-switch proxy failover <p1,p2,...>`
3. 启动代理：`pi-switch proxy start --daemon`
4. 将代理配置添加到 pi：`pi-switch provider add proxy --preset proxy`

</details>

<details>
<summary><b>透明代理路由如何工作？</b></summary>

<br>

当你将一个配置设为代理目标时，其 `baseUrl` 会自动重写为指向本地代理服务器。Pi 的请求会透明地通过代理路由并智能故障转移：
...
现在当 pi 使用 `deepseek-official` 配置时，所有请求都会自动通过代理，在遇到 429/5xx 错误时智能故障转移到 `relay-a` → `relay-b`。

代理会根据模型可用性进行路由——只有在 `models` 列表中包含请求模型的 providers 才会被尝试。
